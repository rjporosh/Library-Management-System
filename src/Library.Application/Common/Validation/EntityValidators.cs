...
public readonly record struct BookCandidate(string? Isbn, string? Title, string? Author, int PublishedYear, string? Description);

/// <summary>Field-level candidate values for a member.</summary>
public readonly record struct MemberCandidate(string? MembershipNumber, string? Name, string? Email);
