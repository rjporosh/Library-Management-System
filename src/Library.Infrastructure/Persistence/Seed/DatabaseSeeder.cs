...
/// <summary>
/// Deterministic, idempotent demo data for a relational database. Reproduces
/// the exact literals the integration tests assert ("Clean Code",
/// 9780132350884, Robert C. Martin) and adds the QA scenarios the spec calls
/// for (available / borrowed / lost copies; active / expiring / inactive
/// members; one overdue borrow).
/// </summary>
public sealed class DatabaseSeeder(LibraryDbContext db)
{
    public async Task SeedAsync(CancellationToken cancellationToken = default)
    {
        ...
        db.Books.AddRange(cleanCode, pragmatic, ddd, refactoring);
        ...
        var bob = new Member(Guid.NewGuid(), "MEM-100002", "Bob Smith", "bob@example.com", now.AddDays(15));
        var charlie = new Member(Guid.NewGuid(), "MEM-100003", "Charlie Brown", "charlie@example.com", now.AddDays(90));
        var dana = new Member(Guid.NewGuid(), "MEM-100004", "Dana Lee", "dana@example.com", now.AddDays(-10));
        dana.Deactivate();
        var erin = new Member(Guid.NewGuid(), "MEM-100005", "Erin Park", "erin@example.com", now.AddDays(120));
        db.Members.AddRange(alice, bob, charlie, dana, erin);
